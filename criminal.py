...
class Criminal:
    def __init__(self,root):
        ...
        #Imag_frame
        img_frame=Frame(self.root,bd=2,relie=RIDGE,bg='white')
        ...
        #2nd
        img_2=Image.open('images/png-transparent-police-officer-west-midlands-police-police-federation-of-england-and-wales-fraternal-order-of-police-policeman-police-officer-people-pedestrian.png')
        ...
        upper_frame.place(x=6,y=10,width=1480,height=270)
        ...
        img_crime=Image.open('images/download.png')
        ...
        down_frame.place(x=6,y=280,width=1480,height=270)
        ...
        search_frame.place(x=2,y=0,width=1470,height=60)
        ...
        crimeagency.grid(row=0,column=5,sticky=W,padx=69,pady=0)
    ...
    def add_data(self):
        if self.var_case_id.get()=="":
            ...
            try:
                conn=mysql.connector.connect(host='localhost',username='root',password='Rockstar@12',database='managment')    
    ...
    def get_cursor(self,event=""):
        ...
        #   'update criminal set Criminal_no=%s,Criminal_name=%s,Nick_name=%s,arrest_date=%s,dateofcrime=%s,address=%s,age=%s,occupation=%s,Birthmark=%s,crime type=%s,fatherName=%s,gender=%s,wanted=%s where Case_id=%s',
    ...
    def update_data(self):
        if self.var_case_id.get()=="":
            ...
            try:
                update=messagebox.askyesno('Update','Are you sure want to update this criminal  record')
                if update>0:
                    conn=mysql.connector.connect(host='localhost',username='root',password='Rockstar@12',database='managment')    
                    my_cursor=conn.cursor()
                
                    my_cursor.execute('update criminal set criminal_no=%s,name=%s,nickname=%s,arrest_date=%s,date_of_crime=%s,address=%s,age=%s,occupation=%s,birthmark=%s,crime_type=%s,father_name=%s,gender=%s,wanted=%s where case_id=%s',(
    ...
    def delete_data(self):
        if self.var_case_id.get()=="":
            ...
            try:
                Delete=messagebox.askyesno('Delete','Are you sure want to Delete this criminal  record')
                if Delete>0:
                    conn=mysql.connector.connect(host='localhost',username='root',password='Rockstar@12',database='managment')    
                    my_cursor=conn.cursor()
                    sql='delete from criminal where Case_id=%s'
                    value=(self.var_case_id(),)
